...
DatSubRaw <- reactive({
    ...
    if(is.null(metricstatselections()$metric)) {
        if(input$Sect_sel != 'FR') {
            metric <- 'Number of vessels'
        } else metric <- 'Number of processors'
    } else metric <-metricstatselections()$metric

    datSubMetric <- datSubforSector[METRIC %in% metric]
